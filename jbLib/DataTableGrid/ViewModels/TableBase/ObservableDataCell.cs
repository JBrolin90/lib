...
internal class ObservableDataCell : INotifyPropertyChanged
{
    ...
    public object? Value
    {
        ...
        set
        {
            try
            {
                if (row[column] != value) row[column] = value ?? DBNull.Value;
            }
            catch (Exception e)
            {
                string columnName = column.ColumnName;
                int rowNumber = row.Table.Rows.IndexOf(row);
                Console.WriteLine($"Error setting value for cell {columnName} in row {rowNumber}: {e.Message}");
            }
